...
let selectedId = '';
export function selectOutput(id: string): void { selectedId = id; }
...
export function onPortChange(fn: (ports: MIDIOutput[]) => void): void { portListeners.push(fn); }
function notifyPortChange(): void { portListeners.forEach(fn => fn(outputs())); }

const beatListeners: Array<(time: number) => void> = [];
export function onBeat(fn: (time: number) => void): void { beatListeners.push(fn); }
