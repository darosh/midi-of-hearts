...
function parseIcon(raw: string): Icon {
  ...
  const vb = vbStr.trim().split(/[\s,]+/).map(Number) as [number, number, number, number];
...
function buildCard(iconsArg: Record<string, Icon>): void {
  ...
  titleG.appendChild(placeIcon(iconsArg.title, titleCX + P * .5, titlePivotY, IH, GUTTER_H));
  ...
  ghLink.appendChild(placeIcon(iconsArg.gh, W - M / 2 - I - ghSize - P * .25, H - M - I / 2, ghSize, ghSize));
  ...
  drumBtn.addEventListener('click', () => { audio.toggleKick(); updateSoundBtns(); });
  drumBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (drumBtn as unknown as HTMLElement).click(); } });
  ...
  sineBtn.addEventListener('click', () => { audio.toggleSine(); updateSoundBtns(); });
  sineBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (sineBtn as unknown as HTMLElement).click(); } });
  ...
  watchBtn.appendChild(placeIcon(iconsArg.onOff, WATCH_C.x, WATCH_C.y, watchSize * .5, watchSize, { id: 'on-off' }));
  watchBtn.addEventListener('click', () => state.isConnected ? bt.disconnect() : bt.connect().catch(console.error));
  watchBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (watchBtn as unknown as HTMLElement).click(); } });
  ...
  const minLabel = el('text', {
    id: 'bpm-min-label',
    x: f(STOP_C.x + labelPerp), y: f(STOP_C.y - labelHalf),
    'font-size': f(SW * 5), 'text-anchor': 'start', 'dominant-baseline': 'auto',
    transform: `rotate(${f(lineAngle + 90)},${f(STOP_C.x)},${f(STOP_C.y)})`,
  });
  const maxLabel = el('text', {
    id: 'bpm-max-label',
    x: f(STOP_C.x - labelPerp), y: f(STOP_C.y - labelHalf),
    'font-size': f(SW * 5), 'text-anchor': 'end', 'dominant-baseline': 'auto',
    transform: `rotate(${f(lineAngle + 90)},${f(STOP_C.x)},${f(STOP_C.y)})`,
  });
  ...
  midiBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (midiBtn as unknown as HTMLElement).click(); } });
  ...
  stopBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (stopBtn as unknown as HTMLElement).click(); } });
  ...
  jackBtn.addEventListener('keydown', e => { if ((e as KeyboardEvent).key === 'Enter' || (e as KeyboardEvent).key === ' ') { e.preventDefault(); (jackBtn as unknown as HTMLElement).click(); } });
...
function openPicker(): void {
  ...
  items.forEach((item, i) => {
    ...
    const label = el('text', {
      ...
      x: f(IX + IW / 2), y: f(y + rowH / 2),
      'text-anchor': 'middle', 'dominant-baseline': 'central', 'font-size': f(fontSize),
    });
...
interface DigitAnim { index: number; maxLen: number; startMs: number }
let digitAnims: DigitAnim[] = [];

interface ArcEvent { startMs: number }
...
function renderBpmChart(now: number): void {
  ...
  const dx = MIDI_C.x - STOP_C.x, dy = MIDI_C.y - STOP_C.y;
...
// ── MIDI port change ──────────────────────────────────────────────────────────
midi.onPortChange(() => { loadStoredOutput(); });
...
export async function init(): Promise<void> {
  ...
  icons = Object.fromEntries(
    Object.entries(rawSvgs).map(([id, raw]) => [id, parseIcon(raw)])
  );
