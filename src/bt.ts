...
export function on(event: BtEvent, fn: BtListener): void { listeners[event]?.push(fn); }

function emit(event: BtEvent, data?: unknown): void { listeners[event]?.forEach(fn => fn(data)); }
